...
def calculateCost(board, start, end, color, correctPlace, searchingAlgorithm):
    ...
    while len(open_list) > 0:
        ...
        for child in children:
            ...
            if color == "R":
                child.g = current_node.g + 1 + totalCost  # for "R" cost is always 1
                

            if color == "G":
                if new_position[0] == -1 or new_position[0] == 1:  # left or right
                    child.g = current_node.g + 1 + totalCost # for "G" left-right cost is 1
                    print("gggggggg",child.g)
                if new_position[1] == -1 or new_position[1] == 1:  # up or down
                    child.g = current_node.g + 2 + totalCost # for "G" up-down cost is 2
                    print("gggggg",child.g)

            if color == "B":
                if new_position[0] == -1 or new_position[0] == 1:  # left or right
                    child.g = current_node.g + 2  + totalCost# for "B" left-right cost is 2
                if new_position[1] == -1 or new_position[1] == 1:  # up or down
                    child.g = current_node.g + 1  + totalCost# for "B" up-down cost is 1

            # child.h = ((child.position[0] - end_node.position[0]) ** 2) + (
            #         (child.position[1] - end_node.position[1]) ** 2)
            if searchingAlgorithm == "2":
                if child.position[0] == end_node.position[0] and child.position[1] == end_node.position[1]:
                    # print("----------------",child.position[0] == end_node.position[0] and child.position[1] == end_node.position[1])
